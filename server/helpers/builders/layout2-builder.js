...
async function Layout2Builder(data) {
  ...
  try {
    ...
    // Substituir placeholders "{{obj.prop.subprop}}"
    content = content.replace(/{([\w.]+)}/g, (match, path) => {
      if (path === 'clientName') return getClientName(data.client);
      return getNestedValue(data, path) || "";
    });

    // Substituir listas "{{#lista}} ... {{/lista}}"
    content = content.replace(/{#(\w+)}([\s\S]*?){\/\1}/g, (match, listKey, listTemplate) => {
      if (Array.isArray(getNestedValue(data, listKey))) {
        return getNestedValue(data, listKey)
          .map((item) => {
            return listTemplate.replace(/{([\w.]+)}/g, (match, key) => getNestedValue(item, key) || "");
          })
          .join(""); // Junta os elementos gerados
      }
      return "";
    });
