...
function StatusItem({ title, status }) {
  ...
  return (
    <div className="rounded-lg overflow-hidden">
      <div className="bg-gray-400 text-white px-3 py-1 text-sm font-medium">{title}</div>
      <div className={`px-3 py-1 flex items-center gap-2 text-sm ${isPositive ? "bg-green-100" : "bg-red-100"}`}>
        {isPositive ? (
  ...
  return (
    <div className="bg-white rounded-xl overflow-hidden shadow-sm border border-[#9D9D9D]">
      ...
      <div className="grid grid-cols-3 gap-4 px-4 py-1">
        {statuses.map((item) => (
          <StatusItem key={item.title} title={item.title} status={item.status} />
        ))}
      </div>
    </div>
  );
