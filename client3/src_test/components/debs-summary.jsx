...
function DebtItem({ label, value, valueColor = "red" }) {
  return (
    <div className="flex flex-col items-start">
      ...
      <span className={`text-sm font-medium ${valueColor === "red" ? "text-red-600" : "text-blue-600"}`}>
  ...
  return (
    <div className="bg-white rounded-xl overflow-hidden shadow-sm border border-[#9D9D9D]">
      ...
      <div className="px-4 py-1 flex flex-wrap gap-6 justify-between">
        {debts.map((debt) => (
          <DebtItem key={debt.label} label={debt.label} value={debt.value} valueColor={debt.valueColor} />
        ))}
      </div>
    </div>
  );
